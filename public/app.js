...
let WorkoutData = []; // Initialize WorkoutData as an empty array

function fetchWorkoutData() {
  fetch("/workout")
    .then((response) => response.json())
    .then((data) => {
      WorkoutData = data; // Update WorkoutData with the fetched data
      // clickStart(); // Call the function to render the initial view
    })
  ...
  for (i = 0; i < WorkoutData[wantedWorkout - 1][wantedNum].sets; i++) {
    ...
    <form>
        <div class="input-container">
          <input name="weightInput" type="number" placeholder="85" />
          <span>kg</span>
        </div>
        <hr />
        <div class="input-container">
          <input name="repsInput" type="number" placeholder="12" />
          <span>reps</span>
        </div>
        <hr />
...
function handleFormSubmit(event) {
  ...
  const workoutData = {
    weight: formData.get("weightInput"), // Replace 'weightInputName' with the actual name of your weight input
    reps: formData.get("repsInput"), // Replace 'repsInputName' with the actual name of your reps input
...
function saveWorkoutData(workoutData) {
  fetch("/workout", {
    ...
    .then((data) => {
      ...
      // You can add code here to handle the response and update the UI accordingly
    })
